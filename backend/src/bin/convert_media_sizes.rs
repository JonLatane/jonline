...
use jonline::logic::{convert_media, media_pending_conversion, ImageMagick};
...
async fn main() {
    ...
    let Some(imagemagick) = ImageMagick::detect() else {
        log::error!(
            "ImageMagick not found on $PATH (tried 'magick', and 'convert'+'identify'). \
             Install it to enable automatic media size conversion; skipping this run."
        );
        std::process::exit(1);
    };
    ...
    for item in pending.iter() {
        ...
        match convert_media(item, &imagemagick, &bucket, tmp_dir.path(), &mut conn).await {
