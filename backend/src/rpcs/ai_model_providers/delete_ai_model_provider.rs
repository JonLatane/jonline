...
pub fn delete_ai_model_provider(
    ...
    let existing = models::get_ai_model_provider(provider_id, conn)?;

    if existing.user_id != current_user.id {
        validate_permission(&Some(current_user), Permission::Admin)?;
    }

    models::delete_ai_model_provider_grants(existing.id, conn)?;

    diesel::delete(ai_model_providers::table.filter(ai_model_providers::id.eq(existing.id)))
        .execute(conn)
        .map_err(|e| {
            log::error!("Failed to delete AI model provider {}: {:?}", existing.id, e);
            Status::new(Code::Internal, "failed_to_delete_ai_model_provider")
        })?;
