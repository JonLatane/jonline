...
pub fn create_event(
    ...
    let result =
        conn.transaction::<(models::Event, models::Post, Vec<(models::EventInstance, Option<models::Post>)>), diesel::result::Error, _>(|conn| {
    ...
    match result {
        Ok((event, post, instances)) => {
            log::info!("Event created! Result: {:?}", event);
            Ok(Response::new(event.to_proto(&post, &instances)))
        }
        Err(e) => {
