...
/// (`UpdateEvent`, `UpdateEventInstances`, `CreateNewEventInstances`, `DeleteRemovedEventInstances`,
...
/// Looks up an `EventInstance` belonging to `event_id` by `instance.post.id` (an `EventInstance`'s
...
/// without including its Post (see `update_event_instances_impl`'s doc for what that means for
...
pub(super) fn find_existing_instance(
    ...
    event_instances::table
        ...
        .filter(event_instances::post_id.eq(instance_post_id))
