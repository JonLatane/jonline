...
pub fn reset_data(user: models::User, conn: &mut PgPooledConnection) -> Result<(), Status> {
    ...
    let result = conn.transaction::<(), diesel::result::Error, _>(|conn| {
        update(media::table)
            ...
            .filter(media::user_id.ne(user.id).or(media::generated.eq(true)))
            .execute(conn)?;
        ...
        delete(users::table)
            ...
            .execute(conn)?;
        update(users::table)
            .set((users::follower_count.eq(0), users::following_count.eq(0)))
            .execute(conn)?;
        Ok(())
    });
    match result {
