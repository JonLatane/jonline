...
pub fn validate_configuration(config: &ServerConfiguration) -> Result<(), Status> {
    ...
    && !config.default_user_permissions.contains(&(Permission::GloballyPublishUsers as i32)) {
        return Err(Status::new(
            Code::NotFound, "global_public_users_require_globally_publish_users_permission",
        ))
    }
    Ok(())
}
