//! Specs for `SyncDestination.synced_occasion_count`/`synced_post_count` -- computed fresh
//! via `COUNT` at request time (see those proto fields' own docs), not stored. Covers
//! `marshaling::attach_synced_counts`/`get_sync_destinations` end to end, for both Occasions
//! and Posts.

use diesel::Connection;

use crate::marshaling::*;
use crate::protos::*;
use crate::rpcs::get_sync_destinations;
use crate::tests::factories::*;

#[test]
fn reports_zero_when_nothing_has_been_synced() {
    let mut conn = test_conn();
    conn.test_transaction::<_, tonic::Status, _>(|conn| {
        let owner = create_user(conn, "sdct_zero");
        create_sync_destination_row(conn, &owner, "123");

        let response =
            get_sync_destinations(User::default(), &owner, conn).expect("get should succeed");
        assert_eq!(response.destinations.len(), 1);
        assert_eq!(
            response.destinations[0].synced_occasion_count,
            Some(0)
        );
        assert_eq!(response.destinations[0].synced_post_count, Some(0));

        Ok(())
    });
}

#[test]
fn counts_synced_instances_for_the_right_destination_only() {
    let mut conn = test_conn();
    conn.test_transaction::<_, tonic::Status, _>(|conn| {
        let owner = create_user(conn, "sdct_count");
        let destination_a = create_sync_destination_row(conn, &owner, "123");
        let destination_b = create_sync_destination_row(conn, &owner, "456");

        let (event, _) = create_event(
            conn,
            &owner,
            EventOpts {
                default_instance: None,
                ..Default::default()
            },
        );
        let (instance_1, _) = create_occasion(conn, &event, Some(&owner), Default::default());
        let (instance_2, _) = create_occasion(conn, &event, Some(&owner), Default::default());

        create_occasion_sync_destination_row(conn, &instance_1, &destination_a);
        create_occasion_sync_destination_row(conn, &instance_2, &destination_a);
        create_occasion_sync_destination_row(conn, &instance_1, &destination_b);

        let response =
            get_sync_destinations(User::default(), &owner, conn).expect("get should succeed");

        let count_for = |db_id: i64| {
            let proto_id = db_id.to_proto_id();
            response
                .destinations
                .iter()
                .find(|d| d.id == proto_id)
                .and_then(|d| d.synced_occasion_count)
        };
        assert_eq!(count_for(destination_a.id), Some(2));
        assert_eq!(count_for(destination_b.id), Some(1));

        Ok(())
    });
}

#[test]
fn counts_synced_posts_for_the_right_destination_only() {
    let mut conn = test_conn();
    conn.test_transaction::<_, tonic::Status, _>(|conn| {
        let owner = create_user(conn, "sdct_post_count");
        let destination_a = create_sync_destination_row(conn, &owner, "123");
        let destination_b = create_sync_destination_row(conn, &owner, "456");

        let post_1 = create_post(conn, Some(&owner), PostOpts::default());
        let post_2 = create_post(conn, Some(&owner), PostOpts::default());

        create_post_sync_destination_row(conn, &post_1, &destination_a);
        create_post_sync_destination_row(conn, &post_2, &destination_a);
        create_post_sync_destination_row(conn, &post_1, &destination_b);

        let response =
            get_sync_destinations(User::default(), &owner, conn).expect("get should succeed");

        let count_for = |db_id: i64| {
            let proto_id = db_id.to_proto_id();
            response
                .destinations
                .iter()
                .find(|d| d.id == proto_id)
                .and_then(|d| d.synced_post_count)
        };
        assert_eq!(count_for(destination_a.id), Some(2));
        assert_eq!(count_for(destination_b.id), Some(1));

        Ok(())
    });
}
