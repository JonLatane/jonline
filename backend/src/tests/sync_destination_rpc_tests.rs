...
fn delete_removes_synced_occasion_and_post_join_rows() {
    ...
    conn.test_transaction::<_, tonic::Status, _>(|conn| {
        ...
        let (instance, _) = create_occasion(conn, &event, Some(&owner), Default::default());
        create_occasion_sync_destination_row(conn, &instance, &destination);
        ...
        let remaining_instance_joins: i64 = occasion_sync_destinations::table
...
fn sync_occasion_requires_sync_events_to_facebook_permission() {
    ...
    conn.test_transaction::<_, tonic::Status, _>(|conn| {
        ...
        let err = sync_occasion(
            SyncOccasionRequest {
                occasion_id: instance.post_id.to_proto_id(),
...
#[test]
fn sync_occasion_fails_for_unknown_instance() {
    let mut conn = test_conn();
    conn.test_transaction::<_, tonic::Status, _>(|conn| {
