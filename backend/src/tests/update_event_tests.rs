//! Specs for `update_event`'s occasion-merging behavior (`update_occasions`, in
//! `rpcs/events/update_event.rs`): given a request's `occasions` list, each entry is matched
//! against the event's existing occasions by `post.id` (an `Occasion`'s identity *is* its own
//! Post's ID -- there's no separate surrogate ID), then either updated in place, created fresh, or
//! (if an existing occasion's `post.id` is missing from the request) deleted. These specs exercise
//! that matching logic directly --
//! `create_event_sets_event_count_once_and_occasion_count_per_occasion` in
...
/// occasion-count/id checks it's convenient for.
#[test]
fn updating_an_existing_occasion_in_place_preserves_its_id_and_persists_changed_fields() {
    ...
    conn.test_transaction::<_, Status, _>(|conn| {
        ...
        assert_eq!(
            ...
            "the existing occasion row should be reused, not replaced with a new id"
        );

        let row =
            occasion_row(conn, occasion.post_id).expect("occasion should still exist");
...
#[test]
fn an_occasion_omitted_from_the_request_is_deleted_but_its_post_survives() {
    let mut conn = test_conn();
    conn.test_transaction::<_, Status, _>(|conn| {
        ...
        assert!(
            ...
            "the occasion present in the request should survive"
        );
        assert!(
            occasion_row(conn, removed.post_id).is_none(),
            "the occasion omitted from the request should be deleted"
        );
        let surviving_post = post_row(conn, removed_post.id);
        assert_eq!(
            surviving_post.id, removed_post.id,
            "the deleted occasion's own Post should be left behind, not cascade-deleted"
        );
...
#[test]
fn an_occasion_with_no_id_in_the_request_creates_a_new_occasion() {
    let mut conn = test_conn();
    conn.test_transaction::<_, Status, _>(|conn| {
        ...
        let updated = update_event(
            Event {
                ...
                occasions: vec![
                    ...
                    Occasion {
                        // No `post` (or a `post` with no `id`) -- brand new occasion.
                        starts_at: Some(new_starts_at.to_proto()),
                        ends_at: Some(new_ends_at.to_proto()),
                        post: Some(Post {
                            visibility: Visibility::ServerPublic as i32,
        ...
        assert_eq!(updated.occasions.len(), 2);
        ...
        let created_id = updated
            ...
            .expect("a second, newly-created occasion should be present");
...
/// simultaneously updates one occasion in place, creates a brand new one, and deletes two others
...
#[test]
fn a_single_call_can_update_create_and_delete_occasions_together() {
    let mut conn = test_conn();
    conn.test_transaction::<_, Status, _>(|conn| {
            ...
            Event {
                ...
                occasions: vec![
                    Occasion {
                        ...
                        ends_at: Some(new_ends_at.to_proto()),
                        post: Some(Post {
                            id: updated_occasion.post_id.to_proto_id(),
                            visibility: Visibility::ServerPublic as i32,
                            ..Default::default()
                        }),
                        ..Default::default()
        ...
        assert_eq!(result.occasions.len(), 2, "1 updated + 1 created");
        assert!(occasion_row(conn, updated_occasion.post_id).is_some());
        ...
        let total: i64 = occasions::table
            ...
            .get_result(conn)
            .unwrap();
        assert_eq!(total, 2, "started with 3 occasions, net +1 -2 = 2");
...
#[test]
fn an_occasion_id_belonging_to_a_different_event_is_not_reassigned() {
    let mut conn = test_conn();
    conn.test_transaction::<_, Status, _>(|conn| {
        ...
        let result = update_event(
            Event {
                ...
                occasions: vec![Occasion {
                    // `occasion_b`'s post id, but submitted under event A.
                    starts_at: Some(occasion_b.starts_at.to_proto()),
                    ends_at: Some(occasion_b.ends_at.to_proto()),
                    post: Some(Post {
                        id: occasion_b.post_id.to_proto_id(),
                        visibility: Visibility::ServerPublic as i32,
                        ..Default::default()
                    }),
                    ..Default::default()
        ...
        assert_eq!(result.occasions.len(), 1);
        let new_occasion_id = result.occasions[0]
            .post
            .as_ref()
            .unwrap()
            .id
            ...
            .unwrap();
        assert_ne!(
            new_occasion_id, occasion_b.post_id,
            "a foreign occasion id should mint a new occasion, not hijack the original"
        );

        let event_a_occasions: i64 = occasions::table
            .filter(occasions::event_id.eq(event_a.post_id))
            .count()
            .get_result(conn)
            .unwrap();
        assert_eq!(event_a_occasions, 1);

        let untouched = occasion_row(conn, occasion_b.post_id)
            .expect("occasion_b should be untouched, not moved or deleted");
        assert_eq!(
            untouched.event_id, event_b.post_id,
            "still belongs to event B"
        );
        assert_eq!(untouched.starts_at, occasion_b.starts_at);

        Ok(())
    });
}

/// Resetting an occasion's Post to `PRIVATE` requires explicitly sending `visibility: PRIVATE` in
...
/// would (dangerously) be treated as a brand new occasion rather than updating the existing one --
/// see `an_occasion_update_with_no_post_creates_a_new_occasion_instead_of_matching_the_existing_one`
/// below for that sharp edge.
#[test]
fn explicitly_setting_an_existing_occasions_post_visibility_to_private_persists_it() {
    let mut conn = test_conn();
    conn.test_transaction::<_, Status, _>(|conn| {
        ...
        let (occasion, occasion_post) = create_occasion(
            conn,
            &event,
            Some(&author),
            OccasionOpts {
        ...
        assert_eq!(occasion_post.visibility, "SERVER_PUBLIC");
        ...
        update_event(
            Event {
                ...
                occasions: vec![Occasion {
                    ...
                    post: Some(Post {
                        id: occasion_post.id.to_proto_id(),
        ...
        let post_after = post_row(conn, occasion_post.id);
...
/// The old "omit `post`" trick no longer identifies an existing occasion -- since identity moved
/// to `post.id`, an update entry with no `post` at all is indistinguishable from a brand new
/// occasion, and `update_event`'s create-before-delete ordering (see its own doc comment) means it
/// gets created rather than silently dropped or matched to the original.
#[test]
fn an_occasion_update_with_no_post_creates_a_new_occasion_instead_of_matching_the_existing_one() {
    let mut conn = test_conn();
    conn.test_transaction::<_, Status, _>(|conn| {
        ...
        assert_eq!(
            updated.occasions.len(),
            1,
            "the original occasion didn't match anything (no post.id) so it was deleted; the \
             post-less entry created a new one in its place"
        );
        let new_post = updated.occasions[0]
            .post
            .as_ref()
            .expect("a post always comes back on read, even for a no-post-override occasion");
        assert_ne!(
            new_post.id,
            occasion_post.id.to_proto_id(),
            "this is a brand new occasion/post, not the original one reset in place"
        );
        assert_eq!(
            new_post.visibility(),
        ...
        assert!(
            occasion_row(conn, occasion.post_id).is_none(),
            "the original occasion is gone -- it didn't match anything in the request"
        );
        let original_post_after = post_row(conn, occasion_post.id);
        assert_eq!(
            original_post_after.visibility, "SERVER_PUBLIC",
            "the original occasion's own Post is untouched (not deleted, not reset to PRIVATE)"
        );
...
/// The comment at `update_event.rs`'s `removed_occasion_owner_ids` explains why this matters: an
/// occasion's own Post can be owned by someone other than the event's author (e.g. an admin
/// editing another user's event), and deleting that occasion -- via omission -- needs to refresh
/// *that* owner's `occasion_count`, not just the acting user's. A second, kept occasion
/// (owned by `event_author`) keeps the event from being left with zero occasions, which is its
/// own separate edge case -- see `deleting_the_only_occasion_leaves_the_event_unretrievable`.
#[test]
fn deleting_an_occasion_owned_by_a_different_user_refreshes_that_users_occasion_count() {
    let mut conn = test_conn();
    conn.test_transaction::<_, Status, _>(|conn| {
        let event_author = create_user(conn, "uet_owner_author");
        let occasion_owner = create_user(conn, "uet_owner_occasion");
        ...
        // `user_counts_tests`.
        diesel::update(users::table.filter(users::id.eq(occasion_owner.id)))
        ...
        let occasion_owner = models::get_user(occasion_owner.id, conn)?;
        assert_eq!(
            occasion_owner.occasion_count, 0,
...
/// occasions simply cannot be selected by it. `update_event` re-reads the event via that same
/// query as its last step (to build the response), so omitting an event's only occasion -- which
...
#[test]
fn deleting_the_only_occasion_leaves_the_event_unretrievable_by_get_events() {
    let mut conn = test_conn();
    conn.test_transaction::<_, Status, _>(|conn| {
        let author = create_user(conn, "uet_lastoccasion_author");
        let author = grant_permissions(conn, &author, vec![Permission::PublishEventsLocally]);
        let (event, event_post) = create_event(conn, &author, EventOpts {
                default_occasion: None,
                ..Default::default()
        ...
        let err = update_event(
            Event {
                ...
                occasions: vec![], // omits `only_occasion` -> deleted
                ..Default::default()
            },
            &author,
            conn,
        ...
        assert!(occasion_row(conn, only_occasion.post_id).is_none());
