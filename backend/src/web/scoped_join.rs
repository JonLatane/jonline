...
//
// TODO: Actually use this in Jonline.
//
// Included in Jonline because https://github.com/kata-containers/kata-containers/tree/main/src/libs/safe-path
// no longer compiles as a whole with Rust stable, but this file does. The Apache license persists for this
// file and this file only in Jonline (which is otherwise AGPL).
//
// That is to say: this particular file, unlike the rest of Jonline, is Apache licensed, not AGPL,
// and can be used in proprietary (closed-source) software; the rest of Jonline's source requires that
// modifications that run on any consumer-facing device or server expose the updated source to those consumers).
