module Pages.User.UserId_.Followers exposing (Model, Msg, fromShared, page)

{-| `/user/:id[@host]/followers` -- the users following a user (looked up by
(permanent) id). Mirrors `Pages.User.UserId_.Posts` exactly --
...
(with `UserListingType.FOLLOWERS`) instead of `Components.Pages.PostsPage`.
-}
...
import Gen.Params.User.UserId_.Followers exposing (Params)
                ...
                Resolver.Loaded user ->
                    let
                        ( listingModel, listingEffect ) =
                            UsersPage.init shared (Just ( newResolver.targetHost, user, FOLLOWERS ))
                    in
