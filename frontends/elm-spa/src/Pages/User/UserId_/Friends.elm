module Pages.User.UserId_.Friends exposing (Model, Msg, fromShared, page)

{-| `/user/:id[@host]/friends` -- the users who mutually follow (and are
followed by) a user (looked up by (permanent) id). Mirrors
`Pages.User.UserId_.Posts` exactly --
...
(with `UserListingType.FRIENDS`) instead of `Components.Pages.PostsPage`.
-}
...
import Gen.Params.User.UserId_.Friends exposing (Params)
                ...
                Resolver.Loaded user ->
                    let
                        ( listingModel, listingEffect ) =
                            UsersPage.init shared (Just ( newResolver.targetHost, user, FRIENDS )) Nothing req.key req.url.path req.query
                    in
