module RellmTests exposing (suite)

{-| Tests for the main Rellm protocol's own multi-server post model -- specifically
...
the actual point of this whole feature: proving a real Rellm `Post` and a translated Mastodon/
...
suite =
    describe "Components.Posts.postTimestamp"
        ...
        , describe "sorting across protocols (the actual point of this feature)"
            [ test "a Rellm post, a translated Mastodon post, and a translated Bluesky post all sort together by time" <|
                \_ ->
                    let
                        ...
                        sortedIds =
                            [ rellmPost, mastodonPost, blueskyPost ]
