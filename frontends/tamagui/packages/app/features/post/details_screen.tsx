...
export function PostDetailsScreen() {
  ...
  const canComment = subjectPost
    && (accountOrServer.account?.user?.permissions?.includes(Permission.REPLY_TO_POSTS)
  ...
  function scrollToBottom() {
    ...
    window.scrollTo({ top: document.body.scrollHeight - windowHeight - 60, behavior: 'smooth' });
  }
  ...
  if (chatUI && (app?.autoRefreshDiscussions ?? true)) {
    if (!_nextChatReplyRefresh || moment().isAfter(_nextChatReplyRefresh)) {
      ...
      const wasAtBottom = isClient && !showScrollPreserver && document.body.scrollHeight - window.innerHeight - window.scrollY < 100;
      setTimeout(() => {
  ...
  return (
    <TabsNavigation selectedGroup={group}>
      <YStack f={1} jc="center" ai="center" mt='$3' marginHorizontal='$3' space w='100%' maw={800}>
        ...
        <ScrollView w='100%'>
          {!subjectPost ? <Paragraph ta="center" fow="800">{`Loading...`}</Paragraph> : undefined}
          ...
          <XStack w='100%'>
            <FlatList data={flattenedReplies}
              ...
              ListFooterComponent={
                <YStack h={showScrollPreserver ? 100000 : chatUI ? 50 : 150} >
